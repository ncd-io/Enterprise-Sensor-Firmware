# Syntax 
# $ python xbee_fota.py <com port> <ncd update file> <PAN ID> <MAC Address> <Chunk size multiple of 16> <FLY Wait 1, No FLY Wait 0> <Start from last segment 1, Start from first segment 0> <PGM Error check Count>
# Example: 
# $ python xbee_fota.py COM4 ./Upgrade.ncd 7FFF 41 DB 74 F9 128 1 1 10
...
GET_PGM_ERROR_STATUS=0x40
GET_CORE_FW_VERSION=0x42
...
def receive_start_fota_cmd_response(address):
    ack = False
    status = 0x00
    param = []
    retry = RETRIALS
    while False == ack and 0 < retry:
        ...
        if (source_addr == address):
            idx = search_frame(payload, '7C')
            if (CMD_RSP_HDR ==  payload[idx]) and (idx + 7) <= len(payload):
                fw_vers = payload[idx + 5]
                status = payload[idx + 6]
                param = payload[(idx + 7): len(payload)]
                ack = True
                if True == ack:
                    break
    return [ack, fw_vers ,status, param]
...
def send_start_fota(address, xbee_obj):
    ...
    if True == ret:
        ...
        while False == ret and 0 < retry:
            ...
            [ret, fw_ver,status, param] = receive_start_fota_cmd_response(address)
            if 0xFF != status:
                ret = False 
                break
...
def read_pgm_error_status(address, xbee_obj):
    cmd = []
    offset = 0
    status = 0
    ret = False
    cmd.append(FOTA_CMD_HDR)
    cmd.append(GET_PGM_ERROR_STATUS)
    cmd.append(0x00)
    cmd.append(0x00)
    cmd.append(0x00)
    ret = False
    ...
    for i in range (0, 2):
        ...
        if 0 != len(payload):
            [ret, status, param] = resp_packet_decoder(address, source_address, payload)
            if 1 == len(param) and (0xFF == status):
                pgm_status = param[0] 
                break
            else:
                print("Tx Failed, Retry")
            ...
            if 1 == len(param) and (0xFF == status):
                ...
                print("Tx Failed, Retry")
                ret = False
    return [ret, pgm_status]

def send_pgm_pkt(address, xbee_obj, pkt_offset, pkt, is_new_fw_ver):
    cmd = []
    ret = False
    status = 0
    cmd.append(FOTA_CMD_HDR)
...
if( 12 == len(sys.argv)):
    pgm_error_check_rate = int(sys.argv[11])
        ...
        if 0x02 == fw_update[image_offset]:
            ...
            while (True):
                ...
                if True == ret:
                    ...
                    if True == ret:
                        print("Starting FOTA")
                        [ret , core_fw_vers] = send_start_fota(address, xbee_obj)
                        if (LAST_OLD_CORE_FW_VERS < core_fw_vers):
                            is_fw_ver_new = True
                    if True == ret:
                        print("FOTA Started") 
                    ...
                    if True == ret: 
                        ...
                        while image_idx < image_length:
                            ...
                            if False == is_fw_ver_new:
                                ret = send_pgm_pkt(address, xbee_obj, image_idx, pkt, False)
                                if True != ret:
                                    break
                            else:
                                send_pgm_pkt(address, xbee_obj, image_idx, pkt, True)
                            image_idx += current_chunk_size
                            if True == is_fw_ver_new:
                                pkt_cnt+=1

                            if True == is_fw_ver_new and pgm_error_check_rate == pkt_cnt or image_idx == image_length:
                                pkt_cnt = 0
                                [ret, last_offset] = send_read_last_segment(address, xbee_obj)	
                                if False == ret:
                                     break
                                else:
                                    if image_idx == image_length and image_length <= last_offset+chunk_size:
                                        reboot = True
                                        break
                                    else:
                                        image_idx = last_offset
